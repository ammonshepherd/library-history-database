        ...
        if file.name.endswith('.htm') and file.is_file():
            ...
            with open(file.path) as file_contents:
                ...
                try:
                    ...
                    # Open a new file in the text_files directory with the same
                    # name as the original file, but with ".txt" added on the
                    # end. So circ1.htm becomes /text_files/circ1.htm.txt
                    new_file = open("./text_files/" + file.name + ".txt", "w")
